...
if($arrConfig["debug"] == 1) {
	echo "\nGet absolute path to backup folder: '" . $arrConfig["path_to_backup"] . "'\n";
}
$temp_path_to_backup = realpath(str_replace('~', $_SERVER['HOME'], $arrConfig["path_to_backup"]));
if (empty($temp_path_to_backup)) {
	echo "\nERROR: Absolute path is invalid: '" . $temp_path_to_backup . "'\n";
	exit("\n*** END ***\n");
}
else {
	$arrConfig["path_to_backup"] = $temp_path_to_backup;
}

if($arrConfig["debug"] == 1) {
	echo "\nAbsolute path to save backup file: '" . $arrConfig["path_to_backup"] . "'\n";
}
...
if(file_exists($arrConfig["path_to_backup"])) {
	...
	if(file_exists($arrConfig["path_to_backup"])) {
		...
		echo "\nERROR: Backup path is invalid: '" . $arrConfig["path_to_backup"] . "'\n";
		exit("\n*** END ***\n");
	}
}
